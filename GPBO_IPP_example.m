...
% initialize OGM with 0.5 occupancy
m = 0.5 * ones(M,N);
...
[extend_list, L] = raycasting(x, sensor, obs_list, extend_list, L, param, step);

%% main loop
P = exp(L)./(ones(M,N) + exp(L));
binMap = ogm2bin(P);
[MI, Hm_0] = cal_MI(P);
ENT_M = [ENT_M; Hm_0];

cover_rate = [0];
temp_cover_rate = compute_cover(P);
...
while ~isempty(action_track) && iteration < param.loopnum 
    ...
    candidate_list = sample_action(extend_list,L,x(:,step), param);
 ...
 %% best action selection
    step = step + 1;
%     MI_GT = action_MI(candidate_list, L, Hm_0, sensor, param); % naive greedy method
    MI_GT = []; % 
    
    if USE_GREEDY
        tic
        MI_best = action_MI(candidate_list, L, Hm_0, sensor, param);
    ...
    elseif USE_GP
        ...
        Y_train = action_MI(X_train, L, Hm_0, sensor, param);
    
        tic
        % GPBO
        [action_best,MI_best] = bayesian_optimization(candidate_list, X_train,Y_train, L, Hm_0, sensor, param, MI_GT);
%       gp_infer_time = toc
    end
    infer_end_time = toc;
    per_infer_time = [per_infer_time; infer_end_time];
